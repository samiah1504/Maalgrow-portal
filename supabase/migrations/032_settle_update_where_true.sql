-- ============================================================
-- 032 — the same guard, the statement I did not check
--
-- 031 fixed the unfiltered DELETE in declare_cycle_profit and I
-- swept for that shape only. The very next statement over the same
-- temp table is an unfiltered UPDATE, which safeupdate refuses in
-- exactly the same way — so settling moved from "DELETE requires a
-- WHERE clause" to "UPDATE requires a WHERE clause" and still wrote
-- nothing. A partial sweep is how you fix a bug twice.
--
-- This time the sweep parsed every UPDATE and DELETE in every
-- migration through to its terminating semicolon and checked each
-- for a WHERE. Three hits, all in this one function, all on the same
-- temp table: the DELETE 031 already fixed, this UPDATE, and 031's
-- own copy of it. Nothing else in the schema.
--
-- WHERE TRUE is the fix. Every row IS meant here — the tax applies
-- to every holder's allocated gross — and saying so explicitly is
-- what the guard asks for. There is no TRUNCATE equivalent for an
-- UPDATE.
--
-- Byte-for-byte the 031 function otherwise, verified by diff.
-- Re-runnable, and supersedes 031.
-- ============================================================
